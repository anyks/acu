...
copyBuildDirectory() {
    ...
    #Copy cellery product to /opt/${PRODUCT}
    mkdir -p "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}"
    chmod -R 755 "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}"

    #Copy cellery product to /opt/${PRODUCT}/bin
    mkdir -p "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}/bin"
    cp -a "$SCRIPTPATH/application/bin/"* "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}/bin/"
    chmod -R 755 "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}/bin"
...
function signProduct() {
    ...
    productsign --sign "Developer ID Installer: Aleksandr Kireev (8NGCRP2X4C)" \
...
function createUninstaller(){
    cp "$SCRIPTPATH/darwin/Resources/uninstall.sh" "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}"
    sed -i '' -e "s/__VERSION__/${VERSION}/g" "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}/uninstall.sh"
    sed -i '' -e "s/__PRODUCT__/${PRODUCT}/g" "${TARGET_DIRECTORY}/darwinpkg/opt/${PRODUCT}/uninstall.sh"
}
